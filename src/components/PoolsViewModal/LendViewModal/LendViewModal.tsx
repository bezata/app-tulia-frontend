...
const LendViewModal = ({ row }: IPoolsViewModalProps) => {
  ...
  const { writeContract: closeDeal, status: closeDealStatus } =
    useWriteContract();

  const { writeContract: defaultLoan, status: defaultLoanStatus } =
    useWriteContract();

  const flashPoolRewards = useGetFlashPoolRewards(row.original.pool);
  useEffect(
    () => {
      if (closeDealStatus === 'success') {
        addAsteroidPoints(account?.address as string, 200, 'closeDeal');
      }
    },
    // @ts-ignore
    { closeDealStatus }
  ...
  const [currentVaultManagerReward, setCurrentVaultManagerReward] =
    useState<number>(0);
  const [flashPoolAllowance, setFlashPoolAllowance] = useState<number>(0);
  ...
  useEffect(() => {
    const fetchAllowance = async () => {
      if (row.original.poolType === 1) {
        // Flash Pool allowance check
        const flashPoolCurrentAllowance = checkAllowance;
        setFlashPoolAllowance(Number(flashPoolCurrentAllowance));
        setApprovalNeeded(
          Number(flashPoolCurrentAllowance) < Number(row.original.amount)
        );
      } else {
        // Standard Pool allowance check
  ...
  useEffect(() => {
    if (defaultLoanStatus === 'success') {
      addAsteroidPoints(account?.address as string, 200, 'defaultLoan');
    }
  }, [defaultLoanStatus]);
