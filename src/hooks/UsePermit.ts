import { erc20Abi } from './../lens/abi/MockTokenCreator';
import { useReadContracts, useSignTypedData } from 'wagmi';
import { useEffect, useMemo, useState } from 'react';
import { computeDomainSeparator } from '@/utils/Permit';
...
function attemptToInferDomain(
  ...
  let domain: {
    ...
    verifyingContract: Address;
    version: string;
  } = { name: name || '', chainId, verifyingContract, version: '1' };
...
export function usePermit(
  ...
  /*//////////////////////////////////////////////////////////////
                            REACT STATE
  //////////////////////////////////////////////////////////////*/
  const [deadline, setDeadline] = useState<uint256>(
    evmCurrentTimePlus(SIGNATURE_SHELF_LIFE)
  );
  ...
  const name = data?.[1];
  const nonce = data?.[2] ?? '';
  const { domain, success: compatible } = attemptToInferDomain(
    // @ts-ignore
    name,
    erc20.chainId,
    erc20.address,
